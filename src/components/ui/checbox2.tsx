...
const checkboxGroupStyles = cva("flex flex-col gap-2");

export function CheckboxGroup2(props: CheckboxGroupProps) {
  const { className, label, children, description, errorMessage, ...rest } =
    props;

  return (
    <AriaCheckboxGroup
      className={cn(checkboxGroupStyles(), className)}
      {...rest}
    >
      <Label>{label}</Label>
      {children}
